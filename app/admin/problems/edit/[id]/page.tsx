...
export default function EditProblem({ params }: { params: { id: string } }) {
  ...
  const fetchProblem = async () => {
    try {
      ...
      if (data.success) {
        const { problem, hints } = data.data;

        const hintsArray = ["", "", ""];
        hints.forEach((hint: Hint) => {
          if (hint.order_index >= 1 && hint.order_index <= 3) {
            hintsArray[hint.order_index - 1] = hint.hint_text;
          }
        });
        ...
        setFormData({
          ...
          correct_answers:
            problem.correct_answers.length > 0 ? problem.correct_answers : [""],
          hints: hintsArray,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.error(data.message || "문제 수정에 실패했습니다.");
      }
    } catch (error) {
